/*
Source File : OutputAESCBCEncodeStream.cpp
...
OutputAESCBCEncodeStream::OutputAESCBCEncodeStream(void)
{
	mTargetStream = NULL;
	mOwnsStream = false;
	mWroteIV = false;
...
OutputAESCBCEncodeStream::OutputAESCBCEncodeStream(
...
LongFilePositionType OutputAESCBCEncodeStream::GetCurrentPosition() 
{
	if (mTargetStream)
...
LongBufferSizeType OutputAESCBCEncodeStream::Write(const IOBasicTypes::Byte* inBuffer, IOBasicTypes::LongBufferSizeType inSize) 
{
	if (!mTargetStream)
		return 0;
...
void OutputAESCBCEncodeStream::Flush() {
