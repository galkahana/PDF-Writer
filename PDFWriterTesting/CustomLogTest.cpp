...
int CustomLogTest(int argc, char* argv[])
{
	...
	do
	{
		...
		status = compressedLogFile.OpenFile(BuildRelativeOutputPath(argv,"CustomLogEncrypted.txt"));
		...
		OutputFile pdfFile;
		status = pdfFile.OpenFile(BuildRelativeOutputPath(argv,"DumpPDFFile.pdf"));
		if(status != PDFHummus::eSuccess)
			break;
		...
		status = decryptedLogFile.OpenFile(BuildRelativeOutputPath(argv,"CustomLogDecrypted.txt"));
		if(status != PDFHummus::eSuccess)
			break;
		...
		status = compressedLogFileInput.OpenFile(BuildRelativeOutputPath(argv,"CustomLogEncrypted.txt"));
		if(status != PDFHummus::eSuccess)
			break;
