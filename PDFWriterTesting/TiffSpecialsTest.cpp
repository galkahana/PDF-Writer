...
static EStatusCode CreatePageForImageAndRelease(PDFWriter& inpdfWriter,PDFFormXObject* inImageObject)
{
...
static EStatusCode CreateBlackAndWhiteMaskImage(char* argv[],PDFWriter& inpdfWriter)
{
    string scJimBW = BuildRelativeInputPath(argv, "images/tiff/jim___ah.tif");
...
static EStatusCode CreateBiLevelGrayScales(char* argv[],PDFWriter& inpdfWriter)
{
	EStatusCode status = PDFHummus::eSuccess;
	TIFFUsageParameters TIFFParameters;
    
    string scWJim = BuildRelativeInputPath(argv, "images/tiff/jim___cg.tif");
...
int  TiffSpecialsTest(int argc, char* argv[])
{
	PDFWriter pdfWriter;
	EStatusCode status; 
	TIFFUsageParameters TIFFParameters;
	...
	do
	{
		status = pdfWriter.StartPDF(BuildRelativeOutputPath(argv, "TiffSpecialsTest.pdf"),ePDFVersion13,LogConfiguration(true,true,BuildRelativeOutputPath(argv, "TiffSpecialsTestLog.txt")));
		if(status != PDFHummus::eSuccess)
		{
		...
		for(int i=0;i<4 && (PDFHummus::eSuccess == status);++i)
		{
			...
			PDFFormXObject* imageFormXObject = pdfWriter.CreateFormXObjectFromTIFFFile(BuildRelativeInputPath(argv, "images/tiff/multipage.tif"),TIFFParameters);
			if(!imageFormXObject)
			{
		...
		status = CreateBlackAndWhiteMaskImage(argv,pdfWriter);
		if(status != PDFHummus::eSuccess)
			break;

		//Create BiLevel grayscales
		status = CreateBiLevelGrayScales(argv,pdfWriter);
		if(status != PDFHummus::eSuccess)
			break;
