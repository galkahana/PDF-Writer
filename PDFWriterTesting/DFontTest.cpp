...
int DFontTest(int argc, char* argv[])
{
	PDFWriter pdfWriter;
	EStatusCode status;
	...
	do
	{
		status = pdfWriter.StartPDF(
                                    BuildRelativeOutputPath(argv,"DFontTest.pdf"),
                                    ePDFVersion13,
                                    LogConfiguration(true,true,BuildRelativeOutputPath(argv,"DFontTest.log")));
		if(status != eSuccess)
		{
        ...
        {
            courierFonts[i] = pdfWriter.GetFontForFile(BuildRelativeInputPath(argv,"fonts/courier.dfont"),i);
            if(!courierFonts[i])
            {
                status = eFailure;
