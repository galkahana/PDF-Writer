...
static EStatusCode TestPageSizeModification(PDFWriter* inPDFWriter)
{
...
static EStatusCode TestAddingComments(PDFWriter* inPDFWriter)
{
    EStatusCode status = eSuccess;
...
int ModifyingExistingFileContent(int argc, char* argv[])
{
    ...
    do 
    {
        ...
        status = pdfWriter.ModifyPDF(
                                     BuildRelativeInputPath(argv,string("AddedPage.pdf")),
                                     ePDFVersion13,
                                     BuildRelativeOutputPath(argv,string("ModifyingExistingFileContent.pdf")),
                                     LogConfiguration(true,true,BuildRelativeOutputPath(argv,string("ModifyingExistingFileContent.log"))));  
		if(status != eSuccess)
		{
