...
static EStatusCode ShowGlobalFontProperties(FreeTypeWrapper& inFreeType,FT_Face inFace)
{
...
int FreeTypeInitializationTest(int argc, char* argv[])
{
	Singleton<Trace>::GetInstance()->SetLogSettings(BuildRelativeOutputPath(argv,"FreeTypeTest.txt"),true,true);
	...
	do
	{
		...
		status = ShowFaceProperties(ftWrapper,
                            BuildRelativeInputPath(argv,"fonts/BrushScriptStd.otf"));
		if(status != PDFHummus::eSuccess)
			break;
