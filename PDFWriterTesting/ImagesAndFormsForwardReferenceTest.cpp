...
int ImagesAndFormsForwardReferenceTest(int argc, char* argv[])
{
	PDFWriter pdfWriter;
	EStatusCode status; 
	...
	do
	{
		...
		status = pdfWriter.StartPDF(BuildRelativeOutputPath(argv,"ImagesAndFormsForwardReferenceTest.pdf"),ePDFVersion13);
		...
		PDFImageXObject* imageXObject  = pdfWriter.CreateImageXObjectFromJPGFile(BuildRelativeInputPath(argv,"images/otherStage.JPG"),imageXObjectID);
		if(!imageXObject)
		{
		...
		PDFFormXObject*  formXObject = pdfWriter.CreateFormXObjectFromJPGFile(
            BuildRelativeInputPath(argv,"images/otherStage.JPG"),formXObjectID);
		if(!formXObject)
		{
		...
		PDFFormXObject* tiffFormXObject = pdfWriter.CreateFormXObjectFromTIFFFile(
            BuildRelativeInputPath(argv,"images/tiff/jim___ah.tif"),tiffFormXObjectID);
		if(!tiffFormXObject)
		{
