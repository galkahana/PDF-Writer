...
int LinksTest(int argc, char* argv[])
{
	EStatusCode status;
	PDFWriter pdfWriter;
	...
	do
	{
		status = pdfWriter.StartPDF(BuildRelativeOutputPath(argv,"LinksTest.pdf"),ePDFVersion13,LogConfiguration(true,true,BuildRelativeOutputPath(argv,"LinksTest.log")));
		...
		PDFFormXObject* soundCloudLogo = pdfWriter.CreateFormXObjectFromJPGFile(
                                                BuildRelativeInputPath(argv,"images/soundcloud_logo.jpg"));
